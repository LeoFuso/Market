...
public class ShoppingServiceImpl implements ShoppingService {
    ...
    public List<Offer> listOffers() {
        return this.orderWebService.retrieveOffers()
                                   .map(resource -> {
                                       final Offer offer = this.converter.convert(resource, Offer.class);
    ...
    public Cart createCart() {
        return this.orderWebService.createCart()
                                   .map(resource -> {

                                       final Tuple2<CartResource, List<ItemResource>> pair =
                                               Tuples.of(resource, Collections.emptyList());
    ...
    private Cart fullCartUsing(final Mono<CartResource> cartResourceProducer, final Long cartId) {
        ...
        final Mono<List<ItemResource>> retrievingItemsProducer = this.orderWebService.itemsFromCart(cartId)
                                                                                     .collectList();
                ...
                .flatMap(cartResponse ->
                                 retrievingItemsProducer
                                         .map(itemsResponse -> {
                                             ...
                                             Tuple2<CartResource, List<ItemResource>> pair =
                                                     Tuples.of(cartResponse, itemsResponse);
